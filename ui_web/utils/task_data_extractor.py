...
class TaskDataExtractor:
    ...
    @staticmethod
    def extract_priority(task: Union[TaskData, 'Task']) -> int:
    ...
    @staticmethod
    def extract_assignee_name(task: Union[TaskData, 'Task']) -> str:
        if not task.assignment or not task.assignment.assignee:
            return ""
        return task.assignment.assignee.display_name

    @staticmethod
    def extract_spent_time_seconds(task: Union[TaskData, 'Task']) -> float:
        ...
        if hasattr(task.time_tracking, 'total_spent_time_days'):
            ...
            if not total_spent_time_days:
                return 0.0
            return Duration.of(total_spent_time_days, TimeUnit.DAY).convert(TimeUnit.SECOND,
                                                                            TimePolicy.BUSINESS_HOURS).time_delta
